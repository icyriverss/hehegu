...
(function(){
    ...
    var prop = {
        ...
        month: '',//所属月份
        method: 'getSellTotal',
        template_url: 'view/yy_jixiao.html',
        title: '山西和合谷{year}年{month}月绩效指标统计表',
        init:function(){
            var today = new Date();
        ...
        initData: function (data) {
            ...
            $.each(this.data.list, function () {
                ...
                var area = getStoreArea(this.ch_branchno);
                this.jiazhishu = dot(div(div(this.total, _this.days), area), 4);
                this.renqi = dot(div(div(this.keliuliang, _this.days), area), 4);
                this.gongshi = div(this.total, this.gongshi);
            });
        },
    };
    my.extend(a, prop);
    my.moudle.yy_jixiao = a;
})();