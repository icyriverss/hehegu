...
(function () {
    var a = new my.table();
    var prop = {
        template_url: 'view/caiwu_order_zg.html',
        title: '历史订货记录',
        perpage: 10,
        page: 1,
        output: 0,
        ...
        bindEvent: function () {
            ...
            $("#submit_order").click(function () {
                ...
                my.view.showTable('caiwu_order_zg');
            });
        },
        passOrder:function(order_id){
            if (confirm("是否确认审核？审核后将不可取消！")) {
                my.util.send('order->passOrder', { order_id: order_id }, function (data) {
                    if (data.status == 1) return my.view.message('审核订单失败！');
                    if (data.status == 0) {
