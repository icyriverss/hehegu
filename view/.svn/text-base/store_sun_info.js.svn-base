...
(function () {
    ...
    var prop = {
        ...
        title: '报损单详情 {order_id}',
        method:'order->getOrderInfo',
        perpage: 10,
        output: 0,
        current_quick: '返回历史报损记录',
        quick: { '返回历史报损记录': 'store_sun_list' },
        real_recv: {},//实收数组
        init: function () {
            this.arges = {
                id: my.tmpOrder_id
        ...
        getTableHtml: function () {
            var _this = this;
            return my.order.getContent(this.data.info, this.data.list, this.data.types, ['bianhao', 'name', 'guige', 'kucun_danwei', 'order_nums','baosun_yuanyin' ]);
        },
        bindEvent: function () {
            var _this = this;
                    ...
                    my.util.send('order->recvOrder', { order_id: my.tmpOrder_id, list: _this.real_recv }, function (data) {
                        ...
                        if (data.status == 0) {
                            my.view.message('确认收货成功！', 1, function () {
                                my.view.showTable('store_order_ok');
                            });
                        }
                    });
                }   
